...
def home_page(request):
    ...
    special_products = Product.objects.filter(is_special=True,status=True).order_by('-created')[:3]
...
def product_detail(request,slug):
    ...
    # similar_product1 = Product.objects.filter(pattern__product=product)
    # print(similar_product1)

    ip_address = request.user.ip_address
    if ip_address not in product.hits.all():
        product.hits.add(ip_address)

    new_order_form = NewOrderFrom(request.POST or None,initial={'product_id':product.id})
...
def filter_products(request):
    ...
    if search is not None:
        products = Product.objects.filter(Q(title__icontains=search) | Q(description__icontains=search) | Q(tag__title__icontains=search),status=True)
